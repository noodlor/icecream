...
if uploaded_file is not None:
    ...
    if run_analysis:
        with st.spinner(UI_TEXT["spinner_running"]):
            ...
            if used_fallback:
                rank_anova = sm.stats.anova_lm(model_rank, typ=2)
                sm_pval = rank_anova.loc['C(Product)', 'PR(>F)']
            ...
            for temp_file in ["temp_sm.csv", "run_sm.R", "temp_sm_pval.txt"]:
        ...
        def render_descriptive():
            if len(attr_names) > 0:
                ...
                # Apply the truncation to the index so the table uses short names
        ...
        def render_driver_analysis():
            if len(attr_names) > 0:
                ...
                if len(valid_corr_cols) > 1:
                    # 1. Global Driver Analysis (Spearman)
                    ...
                    #     # Calculate Spearman, filling NaNs with 0 in case of zero variance
                    ...
                    # # Dynamically scale height based on number of products and attributes
